...
No current `0.1.5` analyzer path emits `IVTS004`. It remains reserved so the diagnostic ID does not need to change if contradictory direct-constraint validation is completed later.
